import { Brackets, LessThanOrEqual, MoreThanOrEqual, ObjectLiteral, SelectQueryBuilder } from 'typeorm';
import { FilterArgs } from '../dtos/filter.dto';
import { OrderSort } from '../types/enums';
import { TypeOrmFilter } from '../types/type';

export abstract class BaseFilter<T extends ObjectLiteral, F extends FilterArgs> implements TypeOrmFilter<T, F> {
  ...
  applySimpleFilters(filters: F): BaseFilter<T, F> {
    for (const [key, value] of Object.entries(filters)) {
      ...
      if (value.constructor === Array) {
        this.query.andWhere(`${this.alias}.${key} IN(:...${key})`, { [key]: value });
      } else {
  ...
  applyOrderBy(filters: F): BaseFilter<T, F> {
    ...
    this.query.orderBy(`${this.alias}.${filters.orderBy}`, filters.order || this.defaultOrderSort);
    return this;
  }
    ...
    this.query.andWhere(
      new Brackets((qb) => {
        this.searchable.forEach((column) => {
          qb.orWhere(`${this.alias}.${String(column)} ILIKE :keyword`, { keyword: `%${filters.keyword}%` });
        });
      }),
    );
